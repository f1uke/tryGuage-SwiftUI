...
//
///https://medium.com/dev-genius/ios-how-create-gaugeview-speedometer-using-swiftui-ae46778e042f
...
struct GaugeView: View {
    ...
    func tickText(at tick: Int, text: String) -> some View {
        ...
        return Text(text).foregroundColor(colorMix(percent: percent)).rotationEffect(.init(degrees: -1 * rotation), anchor: .center).offset(x: -110, y: 0).rotationEffect(Angle.degrees(rotation))
    }
...
struct GaugeView_Previews: PreviewProvider {
    static var previews: some View {
        GaugeView(coveredRadius: 225, maxValue: 100, steperSplit: 10, value: .constant(10.0))
    }
}
